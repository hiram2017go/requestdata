...
namespace HNPatent
{
    ...
    public class HttpHelper
    {
        ...
        //Post数据编码
        private Encoding postencoding = Encoding.UTF8;  //上传中文汉字时出错。修改post编码方式。
        ...
        public string RemoveHtml(string html)
        {
            ...
            return Regex.Replace(Regex.Replace(html, @"<title>.*?</title>", "", RegexOptions.IgnoreCase), "<[^>]+?>", delegate(Match m)
            {
                if (Regex.IsMatch(m.Value, @"<br\s*/?>", RegexOptions.IgnoreCase))
                {
                    return m.Value;
                }
                if (Regex.IsMatch(m.Value, "</?p>", RegexOptions.IgnoreCase))
                {
                    return m.Value;
                }
                else if (Regex.IsMatch(m.Value, "<img.*?>", RegexOptions.IgnoreCase))
                {
                    Regex regex = new Regex(@"<img\s+[^>]*src\s*=\s*(?:(['""])(?<url>[^'""]+)\1|(?<url>\S+))[^>]*>", RegexOptions.IgnoreCase);

                    string value = regex.Match(m.Value).Groups["url"].Value;

                    Regex regex2 = new Regex(@"[a-zA-z]+://[^\s]*.+[a-z]");
                    Match m2 = regex2.Match(value);
                    value = m2.Groups[0].Value;


                    //value 就是图片的地址，根据value的地址就可以将图片下载到本地
        ...
        public void saveimage(string url)
        {
            ...
            string newfilename = DateTime.Now.Year.ToString() + DateTime.Now.Month.ToString() + DateTime.Now.Day.ToString() + DateTime.Now.Hour.ToString() + DateTime.Now.Minute.ToString() + DateTime.Now.Millisecond.ToString() + url;
